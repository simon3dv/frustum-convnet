...
class PointNetModule(nn.Module):
    def __init__(self, Infea, mlp, dist, nsample, use_xyz=True, use_feature=True):
        ...
        self.econv1 = Conv2d(32+2,mlp[0],1)
    ...
    def forward(self, pc, feat, new_pc=None,
                img=None, P=None, query_v1=None, query_uv=None):
        ...
        grouped_uv = torch.gather(
            query_uv, 2,
                indices.view(batch_size, 1, npoint * k).expand(-1, 2, -1)
            ).view(batch_size, 2, npoint, k)

        grouped_rgb = torch.cat([grouped_rgb, grouped_uv],1)
...
class PointNetFeat(nn.Module):
    ...
    def forward(self, point_cloud, sample_pc, feat=None, one_hot_vec=None,
        ...
        feat1 = self.pointnet1(pc, feat, pc1,  img, P, query_v1, query_uv)
        feat1, _ = torch.max(feat1, -1)

        feat2 = self.pointnet2(pc, feat, pc2,  img, P, query_v1, query_uv)
        feat2, _ = torch.max(feat2, -1)

        feat3 = self.pointnet3(pc, feat, pc3,  img, P, query_v1, query_uv)
        feat3, _ = torch.max(feat3, -1)

        feat4 = self.pointnet4(pc, feat, pc4,  img, P, query_v1, query_uv)
...
class PointNetDet(nn.Module):
    ...
    def forward(self,
        ...
        query_uv = data_dicts.get('query_uv')
...
if __name__ == '__main__':
    from datasets.provider_fusion_addgeo import ProviderDataset
